...
ggplot(equality) +
  ...
  geom_smooth(aes(color = "Linear Regression"), se = FALSE, method = "lm", 
                  formula = "y ~ x") +
  scale_fill_manual(values = c( "mediumblue","red2", "mediumpurple2")) +
  paletteer::scale_color_paletteer_d("wesanderson::GrandBudapest1") +
  labs(x = "Percent Urban", y = "Number of LGBTQ+ Laws", color = NULL, fill = "Party") +
  theme(legend.position = "bottom")
...
model_equality_prior_brms <- brm(
  ...
  prior = priors,
  sample_prior = "only",
  chains = 4, iter = 4000, seed = 1234, backend = "cmdstanr", refresh = 0
)
```
...
```{r}
model_equality_brms <- add_criterion(model_equality_brms, criterion = c("loo", "waic"))
...
ggplot(brms_diagnostics) + 
  ...
  paletteer::scale_color_paletteer_d("wesanderson::GrandBudapest1", guide = "none") +
...
```{r}
plot_slopes(model_equality_brms, 
...
p1 <- tibble(humidity9am = seq(0, 100, by = .1)) |> 
  ...
  geom_line(aes(group = .draw), alpha = 0.5, linewidth = 0.5, 
            color = "#d65616") +
...
weather |> 
  ...
  geom_dots(aes(color = raintomorrow, shape = "Data",
...
model_weather_brms |> 
  ...
  ggplot() +
  aes(x = draw_rank, color = factor(.chain)) +
  stat_bin(geom = "step", binwidth = 250, position = position_identity(),
           boundary = 0) +
  paletteer::scale_color_paletteer_d("wesanderson::GrandBudapest1") +
  facet_wrap(~.variable, scales = "free_y") +
...
prediction_brms |> 
  count(.prediction) |> 
  mutate(prop = n / sum(n), 
         prop_nice = scales::percent(prop, accuracy = .1)) |> 
  ggplot() +
...
model_weather_full_brms <- brm(
  ...
  data = weather, family = bernoulli(link = "logit"),
  prior = priors, chains = 4, iter = 4000, seed = 1234, backend = "cmdstanr",
  refresh = 0
)
```

```{r}
expand_grid(humidity9am = mean(weather$humidity9am),
            humidity3pm = seq(min(weather$humidity3pm), 
                              max(weather$humidity3pm), length.out = 100),
            raintoday = c("No", "Yes")) |> 
  add_epred_draws(model_weather_full_brms) |> 
  ggplot() +
  aes(x = humidity3pm, y = .epred, color = raintoday) +
  geom_dots(data = weather, aes(
  ...
  labs(x = "3pm Humidity", y = "Posterior Probability of Rain",
