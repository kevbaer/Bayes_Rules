...
loo_stats <- tribble(
  ...
  mutate(loo = map(model, ~loo(.))) |> 
  mutate(loo_stuff = map(loo, ~as_tibble(.$estimates, 
                                         rownames = "statistic"))) |> 
  select(model_name, loo_stuff) |> 
  unnest(loo_stuff) |> 
  filter(statistic == "elpd_loo") |> 
...
```{r}
loo_compare(loo(model_complete_pooling),
            loo(model_running1), 
            loo(model_running2))
```
...
```{r}
mcmc_plot(model_1_my_priors, variable = "genre", regex = TRUE, prob = .8,
          type = "areas") + 
...
climbers |> 
  ...
  geom_dots(aes(side = if_else(success == TRUE, "bottom", "top")),
            pch = 19, scale = 0.6) +
...
```{r}
# Automatic way
plot_predictions(model_success, condition = "age", re_formula = NA,
                 conf_level = 0.8) + 
  scale_y_continuous(labels = scales::label_percent()) 
```
...
mfx_success <- model_success |> 
  slopes(newdata = datagrid(age = seq(17, 65, by = 1)),
         variables = "age", type = "response", re_formula = NA) |> 
  get_draws()
...
success_classifications <- success_preds |> 
  ungroup() |> 
  summarize(success_prob = mean(.prediction), success_actual = success[1],
            .by = .row) |> 
...
mfx_reviews <- model_reviews |> 
  slopes(newdata = datagrid(model = model_reviews, 
                            rating = airbnb$rating |> unique(),
                            variables = "rating", type = "response",
                            re_formula = NA)) |> 
  get_draws()
