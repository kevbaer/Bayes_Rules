...
```{r}
sim_params <- tibble(type = c("real", "fake"), prior = c(0.6, 0.4))
...
model_election |>
  ...
  labs(
    x = "pi",
    title = "Posterior (Red) + Prior (Gray) for Michelle's support"
  )
```
...
grid_data <- tibble(pi_grid = seq(0, 1, length.out = 6)) |>
  mutate(prior = dbeta(pi_grid, 2, 2), likelihood = dbinom(9, 10, pi_grid)) |>
  mutate(
...
> If the chains are exploring the same space efficiently, the histograms should
> be similar to one another and largely overlapping.
...
"Our 20,000 Markov chain values are about as useful as only 6800 independent
samples (0.34 × 20000)." Happy that this number is greater than .1
...
"Think of Markov chains as a tour around the range of posterior possible values
of a parameter (like µ or π or whatever). The chains move around that parameter
and hopefully converge around it, but the chains need a tour manager to do that
properly."
...
ggplot() +
  stat_function(
    aes(fill = "Prior: Beta (4, 6)"),
    fun = ~ dbeta(., 4, 6),
    geom = "area"
  ) +
  stat_function(
    aes(fill = "Posterior: Beta (18, 92)"),
    fun = ~ dbeta(., 18, 92),
    geom = "area"
  ) +
...
pi_stan_samples |>
  spread_draws(pi) |>
  summarize(
    prop_in_rope = sum(pi > 0.25 & pi < 0.35) / n(),
...
bike_brms |>
  ...
  scale_fill_manual(
    values = c("firebrick3", "grey50", "blue4"),
    guide = "none"
  ) +
...
p1 <- bike_brms |>
  linpred_draws(
    newdata = tibble(temp_feel_c = (75 - temp_details$scaled_center))
  ) |>
  ggplot() +
  aes(x = .linpred) +
  stat_halfeye(fill = "#0188ac") +
  labs(
    title = "Predicted Average Riders\non all days with 75˚",
    x = "Predicted Riders",
    y = NULL
  )
...
one_rep <- bikes |>
  mutate(
    mu = first_draw$b_Intercept + (first_draw$b_temp_feel_c * temp_feel_c),
...
lotsa_reps <- lotsa_draws |>
  mutate(
    mu = map2(b_Intercept, b_temp_feel_c, ~ .x + .y * bikes$temp_feel_c),
...
bike_brms |>
  ...
  aes(x = temp_feel) +
  stat_interval(
    aes(y = .prediction, color_ramp = after_stat(level)),
    alpha = .25,
    .width = c(0.5, 0.9),
    color = "#940034"
  ) +
...
weather_WU <- weather_WU |>
  ...
  mutate(
    across(
      c(temp9am, humidity9am, temp3pm, pressure9am, windspeed9am),
      ~ scale(., scale = FALSE),
      .names = "{col}_centered"
    )
  ) |>
...
p2 <- weather_WU |>
  ...
  geom_line(
    aes(group = paste(location, .draw), color = location),
    alpha = 0.2
  ) +
...
weather_WU |>
  ...
  geom_point(size = .5) +
  geom_line(
    aes(y = .linpred, group = paste(location, .draw)),
    alpha = .2,
    linewidth = .5
  ) +
  paletteer::scale_color_paletteer_d("wesanderson::GrandBudapest1") +
...
loo_brms <- models_brms |>
  mutate(
    loo_stuff = map(loo, ~ as_tibble(.$estimates, rownames = "statistic"))
  ) |>
...
So across our cross validation and elpd, we can see that the temperature +
location is performing quite well.
...
ggplot(equality) +
  ...
  scale_fill_manual(values = c("mediumblue", "red2", "mediumpurple2")) +
  paletteer::scale_color_paletteer_d("wesanderson::GrandBudapest1") +
  labs(
    x = "Percent Urban",
    y = "Number of LGBTQ+ Laws",
    color = NULL,
    fill = "Party"
  ) +
  theme(legend.position = "bottom")
...
prior_draws_brms <- equality |>
  group_by(historical) |>
  summarize(min = min(percent_urban), max = max(percent_urban)) |>
...
prior_draws_brms |>
  ggplot() +
  aes(x = percent_urban, y = .epred) +
  geom_line(
    aes(group = paste(historical, .draw), color = historical),
    alpha = .1,
    linewidth = 0.5
  ) +
  coord_cartesian(ylim = c(0, 100)) +
  scale_color_manual(values = c("mediumblue", "red2", "mediumpurple2")) +
...
model_equality_brms |>
  ...
  mutate(draw_rank = rank(.value)) |>
  ggplot() +
  aes(x = draw_rank, color = factor(.chain)) +
  stat_bin(
    geom = "step",
    binwidth = 250,
    position = position_identity(),
...
```{r}
model_equality_brms <- add_criterion(
  model_equality_brms,
...
brms_diagnostics <- tibble(
  ...
  mutate(highlight = (psis > .5 | p_waic > 1))

ggplot(brms_diagnostics) +
  aes(x = psis, y = p_waic) +
  geom_point(aes(color = highlight)) +
  geom_text_repel(
    data = filter(brms_diagnostics, highlight),
    aes(label = state),
    seed = 1234,
    direction = "y"
  ) +
...
equality |>
  ...
  geom_line(
    aes(y = .epred, group = paste(historical, .draw)),
    linewidth = .5,
    alpha = .3
  ) +
  scale_color_manual(values = c("mediumblue", "red2", "mediumpurple2")) +
  labs(x = "Percent Urban", y = "Number of Laws", color = "Party") +
  theme(legend.position = "bottom")
```
...
```{r}
pp_check(model_pulse_negbinom, ndraws = 50) +
  theme(legend.position = "inside", legend.position.inside = c(.9, .8))
```

```{r}
slopes(model_pulse_negbinom, type = "response") |>
  get_draws() |>
  filter(contrast != "dY/dX") |>
  ggplot() +
  aes(x = draw) +
  stat_halfeye(fill = paletteer::paletteer_d("wesanderson::GrandBudapest1")[2])
```

On average about 2.6 books more amongst Wise than happy.
...
Simulate from priors to see if reasonable

```{r}
priors <- c(
  prior(normal(-1.39, 0.7), class = Intercept),
  prior(normal(0.07, 0.035), class = b, coef = "humidity9am")
)

model_weather_prior_brms <- brm(
  bf(raintomorrow ~ humidity9am),
  data = weather,
...
```{r}
p1 <- tibble(humidity9am = seq(0, 100, by = .1)) |>
  add_epred_draws(model_weather_prior_brms, ndraws = 100) |>
  ggplot() +
  aes(x = humidity9am, y = .epred) +
  geom_line(
    aes(group = .draw),
    alpha = 0.5,
    linewidth = 0.5,
  ...
  labs(x = "9am Humidity", y = "Probability of Rain Tomorrow")

p2 <- tibble(humidity9am = seq(0, 100, by = .1)) |>
  add_predicted_draws(model_weather_prior_brms, ndraws = 100) |>
  ungroup() |>
  summarize(proportion_rain = mean(.prediction == 1), .by = .draw) |>
  ggplot() +
  aes(x = proportion_rain) +
  geom_histogram(binwidth = 0.02, color = "white", fill = "#0188ac") +
...
rain_prob <- weather |>
  ...
  summarize(prop_rain = mean(raintomorrow == "Yes"), .by = humidity_bracket) |>
  arrange(humidity_bracket) |>
  mutate(mid = seq(15, 95, by = 10))

weather |>
  ggplot() +
  aes(x = humidity9am / 100, y = (raintomorrow == "Yes") |> as.numeric()) +
  geom_dots(
    aes(
      color = raintomorrow,
      shape = "Data",
      side = if_else(raintomorrow == "Yes", "bottom", "top")
    ),
    scale = 0.7,
    alpha = 0.5
  ) +
  scale_color_manual(
    values = c("#fb7185", "#2db25f"),
    name = "Rain Tomorrow",
  ...
  geom_point(
    data = rain_prob,
    aes(x = mid / 100, y = prop_rain, shape = "Proportion of Rainy Days")
  ) +
  ggnewscale::new_scale_color() +
  geom_smooth(
    mapping = aes(color = "Probability of Rain"),
    method = "glm",
  ...
  scale_color_manual(values = "#002c55", name = NULL) +
...
prediction_brms <- model_weather_brms |>
  predicted_draws(newdata = tibble(humidity9am = 99))

prediction_brms |>
  count(.prediction) |>
  mutate(prop = n / sum(n), prop_nice = scales::percent(prop, accuracy = .1)) |>
  ggplot() +
  aes(x = factor(.prediction), y = n) +
  geom_col(aes(fill = factor(.prediction))) +
  geom_label(aes(label = prop_nice), nudge_y = -500) +
  scale_fill_manual(values = c("#5c4cbf", "#8ee7af"), guide = "none")
```

```{r}
pp_check(model_weather_brms, ndraws = 100, type = "bars")
```

```{r}
pp_check(model_weather_brms, ndraws = 100, type = "stat", bins = 30) +
...
rain_classifications <- prediction_brms_all |>
  group_by(.row) |>
  summarize(
    rain_prob_that_day = mean(.prediction),
    raintomorrow_actual = raintomorrow[1]
  ) |>
  mutate(raintomorrow_pred = rain_prob_that_day >= .5)
```
...
priors <- c(
  prior(normal(-1.39, 0.7), class = Intercept),
  prior(normal(0, 0.14), class = b, coef = "humidity9am"),
  prior(normal(0, 0.15), class = b, coef = "humidity3pm"),
  prior(normal(0, 6.45), class = b, coef = "raintodayYes")
)

model_weather_full_brms <- brm(
  bf(raintomorrow ~ humidity9am + humidity3pm + raintoday),
  data = weather,
  family = bernoulli(link = "logit"),
  prior = priors,
  chains = 4,
...
```{r}
expand_grid(
  humidity9am = mean(weather$humidity9am),
  humidity3pm = seq(
    min(weather$humidity3pm),
    max(weather$humidity3pm),
    length.out = 100
  ),
  raintoday = c("No", "Yes")
) |>
  ...
  ggplot() +
  aes(x = humidity3pm, y = .epred, color = raintoday) +
  geom_dots(
    data = weather,
    aes(
      y = (raintomorrow == "Yes") |> as.numeric(),
      side = if_else(raintomorrow == "Yes", "bottom", "top")
    ),
    pch = 19,
    color = "grey70",
  ...
  scale_color_manual(values = c("#fb7185", "#2db25f")) +
  scale_fill_manual(values = c("#fb7185", "#2db25f")) +
  labs(
    x = "3pm Humidity",
    y = "Posterior Probability of Rain",
    color = "Rain Today",
    fill = "Rain Today"
  ) +
  scale_y_continuous(labels = scales::label_percent()) +
...
prediction_full |>
  ...
  mutate(
    prop = n / sum(n),
    prop_nice = scales::percent(prop, accuracy = 0.1)
  ) |>
  ggplot() +
  aes(x = factor(.prediction), y = n) +
  geom_col(aes(fill = factor(.prediction))) +
  geom_label(aes(label = prop_nice), nudge_y = -500) +
  scale_fill_manual(values = c("#b3114b", "#ffc517"), guide = "none") +
  facet_wrap(~raintoday)
```

```{r}
pp_check(model_weather_full_brms, ndraws = 100, type = "bars")
```

```{r}
pp_check(model_weather_full_brms, ndraws = 500, type = "stat", bins = 30) +
  labs(x = "Probability of Rain")
```

```{r}
prediction_brms_full_all <- weather |>
  add_predicted_draws(model_weather_full_brms)

rain_classifications_full <- prediction_brms_full_all |>
  group_by(.row) |>
  summarize(
    rain_prob_that_day = mean(.prediction),
    raintomorrow_actual = raintomorrow[1]
...
```{r}
naive_model_1 <- e1071::naiveBayes(
  species ~ bill_length_mm,
  data = penguins_bayes
)

naive_model_2 <- e1071::naiveBayes(
  species ~ bill_length_mm +
    flipper_length_mm,
  data = penguins_bayes
)
...
predict(
  naive_model_1,
  newdata = tibble(bill_length_mm = 50, flipper_length_mm = 195),
  type = "raw"
)
```
...
Typically avoid naive bayes because assumes predictors are independent and all
quantitative predictors are normally distributed.
...
running <- cherry_blossom_sample |>
  mutate(runner_nice = str_glue("Runner {runner}"))
```
...
running |>
  ...
  aes(x = age, y = net) +
  geom_point(alpha = .6) +
  geom_smooth(method = "lm", se = FALSE, color = "#0188ac", formula = "y~x") +
...
model_complete_pooling <- brm(
  ...
  seed = 1234,
  backend = "cmdstanr",
  file = "15_manual_cache/complete_pooling_brms",
  silent = 2,
  refresh = 0
)
```
...
```{r}
tidy(model_complete_pooling, effects = "fixed", conf.level = 0.8)
```

Since the 80% credible interval contains 0, we can't feel confident that age is
impactful.
...
model_complete_pooling |>
  add_epred_draws(
    newdata = tibble(
      age = seq(min(running$age), max(running$age), length.out = 100)
    )
  ) |>
  ggplot() +
  aes(x = age, y = .epred) +
  ...
  stat_lineribbon(alpha = .25, fill = "#2db25f", color = "#2db25f") +
  labs(x = "Age", y = "Finish Time")
```

This doesn't look like it's really appreciating the data.

```{r}
no_pooling <- tibble(runner = levels(running$runner)) |>
  mutate(data = map(runner, ~ filter(running, runner == .x))) |>
  mutate(model = map(data, ~ update(model_complete_pooling, newdata = .x))) |>
  SimDesign::quiet()
```
...
So now we have separate preds for each runner.

Partial pooling / Hierarchical models (happy inbetween!)
...
model_partial_pooling <- brm(
  ...
  backend = "cmdstanr",
  refresh = 0,
  file = "15_manual_cache/partial_pooling"
)
```

```{r}
tidy(model_partial_pooling)
```
...
runner_ages <- running |>
  ...
  mutate(
    age_range = map2(
      age_min,
      age_max,
      ~ tibble(age = seq(.x, .y, length.out = 100))
    )
  ) |>
...
model_partial_pooling |>
  epred_draws(
    newdata = unnest(runner_ages, age_range),
    ndraws = 100,
    re_formula = NULL
  ) |>
  ggplot() +
...
```{r}
spotify |>
  summarize(n_songs = n(), n_artists = n_distinct(artist))
```

```{r}
spotify |> count(artist, sort = TRUE)
```
...
spotify |>
  ggplot() +
  aes(x = popularity, y = artist) +
  geom_point(color = "grey70", size = .75) +
  stat_pointinterval(color = "#d65616", point_interval = "mean_qi") +
...
spotify |>
  ggplot() +
  aes(x = popularity) +
  geom_density(fill = "#940034", color = NA) +
  labs(x = "Popularity", y = "Density") +
  scale_x_continuous(n.breaks = 5, expand = expansion(mult = c(0.1, 0.1)))
```

So looks like reasonable range is from 10 to 90 so mean 50 with sd 20
...
model_pooled <- brm(
  ...
  backend = "cmdstanr",
  refresh = 0,
  file = "16_manual_cache/model_pooled"
)
```
...
pooled_predictions <- spotify |>
  summarize(popularity = mean(popularity), .by = artist) |>
  add_predicted_draws(model_pooled)

pooled_predictions |>
  ggplot() +
  aes(x = .prediction, y = artist) +
  stat_pointinterval(color = "#2db25f", point_interval = "mean_qi") +
  stat_summary(
    data = spotify,
    aes(x = popularity),
    geom = "point",
...
model_no_pooled <- brm(
  ...
  backend = "cmdstanr",
  refresh = 0,
  file = "16_manual_cache/model_no_pooled.rds"
)
```
...
no_pooled_prediction <- spotify |>
  summarize(popularity = mean(popularity), .by = artist) |>
  add_predicted_draws(model_no_pooled)

no_pooled_prediction |>
  ggplot() +
  aes(x = .prediction, y = artist) +
  stat_pointinterval(color = "#2db25f", point_interval = "mean_qi") +
  stat_summary(
...
model_multilevel <- brm(
  ...
  backend = "cmdstanr",
  refresh = 0,
  file = "16_manual_cache/model_multilevel.rds"
)
```
...
```{r}
model_multilevel |>
  tidy(effects = "ran_vals", conf.level = .8) |>
...
model_multilevel |>
  ...
  ggplot() +
  aes(x = mu_j, y = artist) +
  stat_pointinterval(color = "#d65616") +
  labs(x = "Popularity", y = NULL)
```
...
model_multilevel |>
  ...
  ggplot() +
  aes(x = .prediction) +
  stat_halfeye(fill = "#fb7185") +
  labs(x = "Posterior for Popularity of Next Frank Ocean Song")
```
...
model_multilevel |>
  ...
  ggplot() +
  aes(x = .linpred) +
  stat_halfeye(fill = "#adb4f4") +
  labs(x = "Posterior for Average Popularity of Frank Ocean's Songs")
```

```{r}
model_multilevel |>
  predicted_draws(
    newdata = tibble(artist = "Mohsen Beats"),
    re_formula = NULL,
  ...
  ggplot() +
  aes(x = .prediction) +
  stat_halfeye(fill = "#b2e3ff") +
  labs(x = "Posterior for Popularity of Next Mohsen Beats' Song")
```
...
model_multilevel |>
  ...
  ggplot() +
  aes(x = .linpred) +
  stat_halfeye(fill = "#002c55") +
  labs(x = "Posterior for Average Popularity of Mohsen Beats' Songs")
```
...
p1 <- model_multilevel |>
  predicted_draws(
    newdata = tibble(artist = c("Frank Ocean", "Mohsen Beats")),
    re_formula = NULL,
    allow_new_levels = TRUE,
    sample_new_levels = "gaussian"
  ) |>
  ggplot() +
  aes(x = .prediction, y = artist, fill = artist) +
  stat_halfeye() +
  scale_fill_manual(values = c("#940034", "#2db25f"), guide = "none") +
  labs(x = "Posterior for Individual Songs", y = NULL)
...
p2 <- model_multilevel |>
  ...
  ggplot() +
  aes(x = .linpred, y = artist, fill = artist) +
  stat_halfeye() +
  scale_fill_manual(values = c("#940034", "#2db25f"), guide = "none") +
  labs(x = "Posterior for Average of All Songs", y = NULL) +
...
multilevel_predictions <- model_multilevel |>
  predicted_draws(newdata = tibble(artist = spotify$artist |> unique()))

multilevel_predictions |>
  ggplot() +
  aes(x = .prediction, y = artist) +
  stat_pointinterval(color = "#0188ac", point_interval = "mean_qi") +
  stat_summary(
    data = spotify,
    aes(x = popularity),
    color = "#ffc517",
...
running <- cherry_blossom_sample |>
  ...
  drop_na() |>
  mutate(runner_nice = str_glue("Runner {runner}")) |>
  mutate(runner_nice = fct_inorder(runner_nice))
```
...
model_complete_pooling <- brm(
  ...
  refresh = 0,
  backend = "cmdstanr",
  file = "17_manual_cache/complete_pooled"
)
```
...
model_complete_pooling |>
  ...
  stat_lineribbon(alpha = 0.25, fill = "#002c55", color = "#002c55") +
  labs(x = "Age", y = "Finish Time")
```
...
model_running1_prior <- brm(
  ...
  backend = "cmdstanr",
  refresh = 0,
  file = "17_manual_cache/running1_prior"
)
```

```{r}
p1 <- running |>
  add_predicted_draws(model_running1_prior, ndraws = 100) |>
  ggplot() +
  aes(x = .prediction, group = .draw) +
  geom_density(linewidth = 0.25, col = "#940034")

p2 <- running |>
  add_linpred_draws(model_running1_prior, ndraws = 6, seed = 11042004) |>
  ggplot() +
  aes(x = age, y = net) +
  geom_line(
    aes(y = .linpred, group = paste(runner, .draw)),
    color = "#002c55",
    linewidth = .25
  ) +
...
(p1 | p2) + plot_layout(widths = c(.3, .7))
```
...
model_running1 <- brm(
  ...
  refresh = 0,
  backend = "cmdstanr",
  file = "17_manual_cache/running1"
)
```

```{r}
model_running1 |>
  tidy(effects = c("fixed"), conf.level = .8) |>
  select(-component)
```
...
running1_runner_offsets <- model_running1 |>
  linpred_draws(
    newdata = tibble(
      runner = unique(running$runner),
      age = running$age |> mean()
    )
  ) |>
  ungroup() |>
...
running1_runner_offsets |>
  ggplot() +
  aes(x = .linpred, y = runner) +
  stat_pointinterval(color = "#002c55")
```

```{r}
running |>
  filter(runner %in% c("4", "5")) |>
  add_linpred_draws(model_running1, ndraws = 100) |>
  ggplot() +
  aes(x = age, y = net) +
  geom_line(
  ...
  scale_color_manual(values = c("#d65616", "#0188ac"))
```

```{r}
performance::icc(model_running1, by_group = TRUE)
```
...
p2 <- running |>
  add_linpred_draws(model_running1, ndraws = 100) |>
  ungroup() |>
  summarize(.linpred = mean(.linpred), .by = c(runner, age)) |>
  ggplot() +
  aes(x = age, y = net) +
  geom_line(
    aes(y = .linpred, group = runner),
    linewidth = 0.5,
    color = "#0188ac"
  ) +
  coord_cartesian(ylim = c(60, 125)) +
...
```{r}
performance::icc(model_running2)
```

```{r}
running2_slope_int_cor <- model_running2 |>
  spread_draws(b_Intercept, b_age, r_runner[runner, term]) |>
  pivot_wider(names_from = "term", values_from = r_runner) |>
  mutate(runner_intercept = b_Intercept + Intercept, runner_age = b_age) |>
  group_by(runner) |>
  summarize(
    correlation = cor(runner_intercept, runner_age),
...
running2_slope_int_cor |>
  ...
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "#2db25f", formula = "y~x") +
...
p1 <- running |>
  add_linpred_draws(model_complete_pooling, ndraws = 100) |>
  ungroup() |>
  summarize(.linpred = mean(.linpred), .by = c(runner, age)) |>
  ...
  coord_cartesian(ylim = c(60, 125)) +
  labs(x = "Age", y = "Race time", title = "Completely pooled model")
...
p2 <- running |>
  ...
  coord_cartesian(ylim = c(60, 125)) +
  labs(x = "Age", y = NULL, title = "Random intercepts only")

p3 <- running |>
  add_linpred_draws(model_running2, ndraws = 100) |>
  ungroup() |>
  summarize(.linpred = mean(.linpred), .by = c(runner, age)) |>
  ggplot(aes(x = age, y = net)) +
  geom_line(
  ...
  coord_cartesian(ylim = c(60, 125)) +
  labs(x = "Age", y = NULL, title = "Random intercepts/slopes")
...
p1 <- pp_check(model_complete_pooling, ndraws = 25, type = "dens_overlay") +
  labs(title = "Completely Pooled Model")

p2 <- pp_check(model_running1, ndraws = 25, type = "dens_overlay") +
  labs(title = "Random Intercepts Only")

p3 <- pp_check(model_running2, ndraws = 25, type = "dens_overlay") +
  labs(title = "Random Intercepts/Slopes")

(p1 | p2 | p3) +
  plot_layout(guides = "collect", axes = "collect") &
  theme(legend.position = "bottom") &
  coord_cartesian(c(45, 135)) &
  labs(x = "Finish Time")
```
...
loo_stats <- tribble(
  ~model_name                    , ~model                 ,
  "Completely pooled model"      , model_complete_pooling ,
  "Random intercepts only"       , model_running1         ,
  "Random intercepts and slopes" , model_running2
) |>
  mutate(loo = map(model, ~ loo(.))) |>
  mutate(
    loo_stuff = map(loo, ~ as_tibble(.$estimates, rownames = "statistic"))
...
```{r}
priors <- c(
  prior(normal(50, 2.5), class = Intercept),
  prior(normal(0, 2.5), class = b),
  prior(exponential(1), class = sigma),
  prior(exponential(1), class = sd)
)

model_1_default_priors <- brm(
  bf(danceability ~ valence + genre + (1 | artist)),
  data = spotify,
  family = gaussian(),
  chains = 4,
  cores = 4,
  ...
  threads = threading(2),
  seed = 1234,
  file = "17_manual_cache/spotify_1_default",
  backend = "cmdstanr",
  refresh = 0
)

model_1_my_priors <- brm(
  bf(danceability ~ valence + genre + (1 | artist)),
  data = spotify,
  family = gaussian(),
  chains = 4,
  ...
  prior = priors,
  seed = 1234,
  file = "17_manual_cache/spotify_1_set",
  backend = "cmdstanr",
  refresh = 0
)

model_2_default_priors <- brm(
  bf(danceability ~ valence + genre + (valence | artist)),
  data = spotify,
  family = gaussian(),
  chains = 4,
  cores = 4,
  ...
  threads = threading(2),
  seed = 1234,
  file = "17_manual_cache/spotify_2_default",
  backend = "cmdstanr",
  refresh = 0
)
...
model_2_my_priors <- brm(
  ...
  prior = priors,
  seed = 1234,
  file = "17_manual_cache/spotify_2_set",
  backend = "cmdstanr",
  refresh = 0
)
```
...
```{r}
pp_check(model_1_default_priors, ndraws = 25)
```

```{r}
pp_check(model_1_my_priors, ndraws = 25)
```

```{r}
pp_check(model_2_default_priors, ndraws = 25)
```

```{r}
pp_check(model_2_my_priors, ndraws = 25)
```

```{r}
loo_compare(
  loo(model_1_default_priors),
  loo(model_1_my_priors),
  loo(model_2_default_priors),
  loo(model_2_my_priors)
)
```
...
loo_stats <- tribble(
  ~model_name               , ~model                 ,
  "Model 1, Default Priors" , model_1_default_priors ,
  "Model 1, My Priors"      , model_1_my_priors      ,
  "Model 2, Default Priors" , model_2_default_priors ,
  "Model 2, My Priors"      , model_2_my_priors
) |>
  mutate(loo = map(model, ~ loo(.))) |>
  mutate(
    loo_stuff = map(loo, ~ as_tibble(.$estimates, rownames = "statistic"))
...
```{r}
tidy(model_1_my_priors, effects = "fixed", conf.level = .8)
```
...
mcmc_plot(
  ...
  geom_vline(xintercept = 0, color = "#b3114b")
```

## Chapter 18: Non-Normal Hierarchical Regression & Classificiation
...
climbers |>
  count(success) |>
  mutate(prop = (n / sum(n)) |> scales::percent(accuracy = 0.01))
```

```{r}
climbers |>
  ggplot() +
  aes(x = age, y = success, color = oxygen_used) +
  geom_dots(
    aes(side = if_else(success == TRUE, "bottom", "top")),
    pch = 19,
    scale = 0.6
  ) +
...
model_success <- brm(
  bf(success ~ age + oxygen_used + (1 | expedition_id)),
  family = bernoulli(),
  data = climbers,
  prior = priors,
  chains = 4,
  ...
  cores = 4,
  backend = "cmdstanr",
  file = "18_manual_cache/model_success",
  refresh = 0,
  seed = 1234
)
```
...
datagrid(model = model_success, age = seq(17, 65, by = 1)) |>
  add_epred_draws(model_success, re_formula = NA) |>
  ggplot() +
  aes(x = age, y = .epred) +
  stat_lineribbon(color = "#002c55", fill = "#b2e3ff", alpha = .35) +
  scale_y_continuous(labels = scales::label_percent())
```

```{r}
mfx_success <- model_success |>
  slopes(
    newdata = datagrid(age = seq(17, 65, by = 1)),
    variables = "age",
    type = "response",
    re_formula = NA
  ) |>
...
mfx_success |>
  ggplot() +
  aes(x = age, y = draw) +
  stat_lineribbon(alpha = 0.35, fill = "#d65616", color = "#d65616") +
  labs(x = "Age", y = "Change in Success Probability") +
  scale_y_continuous(labels = scales::label_percent())
```

```{r}
datagrid(model = model_success, oxygen_used = c(TRUE, FALSE)) |>
  add_epred_draws(model_success, re_formula = NA) |>
  ggplot() +
  aes(x = oxygen_used, y = .epred, fill = oxygen_used, color = oxygen_used) +
  stat_gradientinterval(width = .25) +
  scale_y_continuous(labels = scales::label_percent()) +
  scale_fill_manual(values = c("#fda4d0", "#8ee7af"), guide = "none") +
  scale_color_manual(values = c("#b3114b", "#2db25f"), guide = "none") +
  labs(x = "Oxygen Used", y = "Posterior Probability of Success")
```

```{r}
performance::icc(model_success, by_group = TRUE)
```

```{r}
pp_check(model_success, ndraws = 100, type = "bars")
```

```{r}
pp_check(model_success, type = "stat", bins = 30) +
  labs(x = "Probability of success")
```

```{r}
success_preds <- model_success |>
  predicted_draws(climbers)

success_classifications <- success_preds |>
  ungroup() |>
  summarize(
    success_prob = mean(.prediction),
    success_actual = success[1],
    .by = .row
  ) |>
  mutate(success_pred = success_prob > .5)
...
priors <- c(
  ...
  prior(normal(0, 2.5), class = b, coef = "rating"),
...
model_reviews <- brm(
  bf(reviews ~ rating + room_type + (1 | neighborhood)),
  data = airbnb,
  prior = priors,
  family = negbinomial(),
  cores = 4,
  ...
  backend = "cmdstanr",
  refresh = 0,
  file = "18_manual_cache/model_reviews"
)
```
...
```{r}
mfx_reviews <- model_reviews |>
  slopes(
    newdata = datagrid(
      model = model_reviews,
      rating = airbnb$rating |> unique(),
      variables = "rating",
      type = "response",
      re_formula = NA
    )
...
mfx_reviews |>
  ggplot() +
  aes(x = rating, y = draw) +
  stat_lineribbon(alpha = .25, fill = "#0188ac", color = "#0188ac")
```

Doesn't look great...

```{r}
pp_check(model_reviews, ndraws = 25)
```
...
airbnb |>
  ggplot() +
  aes(x = price) +
  geom_histogram(binwidth = 20, fill = "#5c4cfb", color = "white")
```
...
airbnb |>
  ggplot() +
  aes(x = log(price)) +
  geom_histogram(binwidth = .25, fill = "#5c4cfb", color = "white")
```

Woah, now it looks kinda normalish

```{r}
priors <- c(
  prior(normal(4.6, 2.5), class = Intercept),
  prior(normal(0, 2.5), class = b),
  prior(exponential(1), class = sd)
)

airbnb_model_1 <- brm(
  bf(log(price) ~ bedrooms + rating + room_type + (1 | neighborhood)),
  data = airbnb,
  family = gaussian(),
  prior = priors,
  cores = 4,
  ...
  seed = 1234,
  backend = "cmdstanr",
  file = "19_manual_cache/airbnb_model_1",
  refresh = 0,
  threads = threading(2)
)
```
...
```{r}
pp_check(airbnb_model_1, ndraws = 25) +
  theme(legend.position = "inside", legend.position.inside = c(0.9, 0.85))
```

Doesn't look half-bad.

But we also have predictors like walk_score which are the same for each
neighborhood but impact the understanding
...
airbnb_model_2 <- brm(
  ...
  seed = 1234,
  backend = "cmdstanr",
  file = "19_manual_cache/airbnb_model_2",
  refresh = 0,
  threads = threading(2)
)
...
```{r}
pp_check(airbnb_model_2, ndraws = 25) +
...
climb_model_1 <- brm(
  ...
  cores = 4,
  backend = "cmdstanr",
  file = "19_manual_cache/climb_model_1",
  refresh = 0,
  seed = 1234
)

climb_model_2 <- brm(
  bf(success ~ age + oxygen_used + (1 | expedition_id) + (1 | peak_name)),
  family = bernoulli(),
  data = climbers,
  prior = priors,
  chains = 4,
  ...
  cores = 4,
  backend = "cmdstanr",
  file = "19_manual_cache/climb_model_2",
  refresh = 0,
  seed = 1234
)
```
...
```{r}
pp_check(climb_model_1, type = "bars", ndraws = 25)
```

```{r}
pp_check(climb_model_2, type = "bars", ndraws = 25)
```
