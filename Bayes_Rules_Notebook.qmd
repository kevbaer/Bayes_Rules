...
naive_model_2 <- e1071::naiveBayes(species ~ bill_length_mm + 
                                     flipper_length_mm, 
                                   data = penguins_bayes)
```

```{r}
predict(naive_model_1, newdata = tibble(bill_length_mm = 50,
                                        flipper_length_mm = 195),
        type = "raw")
```

```{r}
predict(naive_model_2, newdata = tibble(bill_length_mm = 50,
                                        flipper_length_mm = 195),
        type = "raw")
```
...
model_complete_pooling <- brm(
  ...
  family = gaussian(),
  prior = priors,
  chains = 4, cores = 4, iter = 4000, seed = 1234, backend = "cmdstanr",
...
model_complete_pooling |> 
  ...
  geom_smooth(data = running |> 
                drop_na(net), 
              aes(y = net, group = runner), method = "lm",
              se = FALSE, formula = "y~x", color = "grey60", linewidth = .5) +
...
model_partial_pooling <- brm(
  ...
  family = gaussian(),
  prior = priors,
  chains = 4, cores = 4, iter = 8000, threads = threading(2), seed = 1234,
  backend = "cmdstanr", refresh = 0, file = "15_manual_cache/partial_pooling"
)
```
...
model_partial_pooling |> 
  epred_draws(newdata = unnest(runner_ages, age_range),
              ndraws = 100, re_formula = NULL) |> 
  ggplot() +
  aes(x = age, y = .epred) +
  geom_line(aes(group = paste(runner, .draw)), linewidth = .1,
            color = "#940034", alpha = .5) +
  geom_point(data = running |> 
               drop_na(net), aes(y = net), size = .75, color = "#0188ac",
             alpha = .8) +
  labs(x = "Age", y = "Finish Time") +
  facet_wrap(~runner, ncol = 4, labeller = as_labeller(
    \(x) str_glue("Runner {x}")))
```
...
model_pooled <- brm(
  ...
  prior = priors,
  chains = 4, cores = 4, iter = 4000, seed = 1234, backend = "cmdstanr",
  refresh = 0, file = "16_manual_cache/model_pooled"
)
```
...
pooled_predictions |> 
  ...
  stat_summary(data = spotify, aes(x = popularity), geom = "point",
               fun = mean, color = "#d65616", size = 3, pch = 18) +
  labs(x = "Popularity", y = NULL)
```
...
model_no_pooled <- brm(
  ...
  family = "gaussian",
  prior = priors,
  chains = 4, cores = 4, seed = 1234, iter = 4000, backend = "cmdstanr",
  refresh = 0, file = "16_manual_cache/model_no_pooled.rds"
)
```
...
no_pooled_prediction |> 
  ...
  aes(x = .prediction, y = artist) +
  stat_pointinterval(color = "#2db25f", point_interval = "mean_qi") +
  stat_summary(data = spotify, color = "#d65616", geom = "point",
               aes(x = popularity), fun = mean, size = 3, pch = 18) +
  labs(x = "Popularity", y = NULL)
```
...
model_multilevel <- brm(
  ...
  data = spotify, family = "gaussian", prior = priors,
  cores = 4, iter = 4000, chains = 4, seed = 1234, backend = "cmdstanr",
  refresh = 0, file = "16_manual_cache/model_multilevel.rds"
)
```
...
multilevel_predictions |> 
  ...
  stat_summary(data = spotify, aes(x = popularity), color = "#ffc517",
               pch = 18, size = 3, geom = "point", fun = mean) +
  labs(x = "Popularity", y = NULL)
```
